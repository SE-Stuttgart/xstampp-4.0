...
import { FEEDBACK, FeedbackResponseDTO, UnlockRequestDTO } from './../../types/local-types';
...
export class FeedbackComponent implements OnDestroy {
  ...
  feedbacks = [];
  columns: TableColumn[] = [];
  feedback: { id: string, name: string, description: string, state: string, };
  sheetMode: SheetMode = SheetMode.Closed;
  fields: DetailedField[];
  title: string = DetailedSheetUtils.generateSheetTitle(FEEDBACK);
  private OwnChange: boolean = false;
  disableSaveButton: boolean = false;
  subscriptions: Subscription[] = [];
  selectFeedbackEntity: SelectedEntity<Object>;
  ...
  constructor(
    ...
    this.subscriptions.push(this.hotkeys.addShortcut({ keys: 'shift.control.n', description: 'Create Feedback' }).subscribe(() => {
      this.createFeedback();
    }));
    this.fields = [
    ...
    this.deleteFeedbacks = this.deleteFeedbacks.bind(this);
    ...
    this.authService.getProjectToken().then((token: string) => {
      this.wsService.connect('subscribe', FEEDBACK, token, (data) => {
        this.loadData();
  ...
  async deleteFeedbacks($event) {
    ...
    const promiseList = list.map(async (elem: any) => {
      ...
      return await this.feedbackDataService.deleteFeedback(this.projectId, elem.id);
    });
  ...
  createFeedback(): void {
    ...
    this.feedback = { id: '', description: '', name: '', state: '' };
  }

  saveFeedback(feedback): void {
    this.filterService.ClearSelectionEmitter.emit(true);
    this.OwnChange = true;
    this.disableSaveButton = true;
    if (!feedback.ent.name || feedback.ent.name.trim() === '') {
      this.messageService.add({ severity: 'warn', summary: DETAILEDSHEET_EXPECTED_NAME_NOT_NULL })
      this.disableSaveButton = false;
      return;
    }
    if (feedback.mode === SheetMode.EditWithLock) {
      this.feedbackDataService.alterFeedback(this.projectId, feedback.id, feedback.ent)
        .then(() => {
    ...
    } else if (feedback.mode === SheetMode.New) {
      this.feedbackDataService.createFeedback(this.projectId, feedback.ent)
        .then(() => {
  ...
  getSelectedEntities($event: any): void {
    this.selectFeedbackEntity = new SelectedEntity<Object>($event,
      this.title,
      this.projectId,
      false,
      false,
  ...
  changeEntity(event: { list: FeedbackResponseDTO[], state: string }) {
    let incompleteEntity: incompleteEntityDTO = { entityName: 'feedback', parentId: undefined, state: event.state };
    event.list.forEach((element: FeedbackResponseDTO) => {
  ...
  private loadData(): void {
    ...
    this.feedbackDataService.getAllFeedbacksForTable(this.projectId, {'orderBy': 'id', 'orderDirection': 'asc'}).then(value => {
      if(value !== null){
  ...
  closeSheet(isUnlocked: Boolean = false, sheetMode: SheetMode = SheetMode.Closed): void {
    if (!isUnlocked && this.sheetMode === SheetMode.Edit) {
      const unlockDTO: UnlockRequestDTO = { id: this.feedback.id, entityName: this.title.toLowerCase() };
