...
import { ACTUATOR, ActuatorResponseDTO, UnlockRequestDTO } from '../../types/local-types';

import { DETAILEDSHEET_EXPECTED_NAME_NOT_NULL, ENTITY_FAILED_DELETE, ENTITY_SUCCESSFUL_DELETE, DETAILEDSHEET_SUCCESSFUL_EDIT, DETAILEDSHEET_FAILED_EDIT, DETAILEDSHEET_SUCCESSFUL_CREATION, DETAILEDSHEET_FAILED_CREATION, DELETED_USER, ENTITY_FAILED_LOADING, CHANGED_STATE_FAILED, NO_USER } from 'src/app/globals';
...
export class ActuatorComponent implements OnDestroy {

  SheetMode: typeof SheetMode = SheetMode;
  private projectId: string;
  actuators = [];
  columns: TableColumn[] = [];
  actuator: { id: string, name: string, description: string, state: string };
  sheetMode: SheetMode = SheetMode.Closed;
  fields: DetailedField[];
  private OwnChange: boolean = false;
  disableSaveButton: boolean = false;
  title: string = DetailedSheetUtils.generateSheetTitle(ACTUATOR);
  subscriptions: Subscription[] = [];
  selectActuatorEntity: SelectedEntity<Object>;
  ...
  constructor(
    ...
    this.subscriptions.push(this.hotkeys.addShortcut({ keys: 'shift.control.n', description: 'Create Actuator' }).subscribe(() => {
      this.createActuator();
    }));
    ...
    this.deleteActuators = this.deleteActuators.bind(this);
    ...
    this.authService.getProjectToken().then((token: string) => {
      this.wsService.connect('subscribe', ACTUATOR, token, (data) => {
        this.loadData();
  ...
  async deleteActuators($event: any): Promise<void> {
    this.OwnChange = true;
  ...
  createActuator(): void {
    ...
    this.actuator = { id: '', description: '', name: '', state: '' };
  }

  saveActuator(actuator): void {
    this.filterService.ClearSelectionEmitter.emit(true);
    this.OwnChange = true;
    this.disableSaveButton = true;
    if (!actuator.ent.name || actuator.ent.name.trim() === '') {
      this.messageService.add({ severity: 'warn', summary: DETAILEDSHEET_EXPECTED_NAME_NOT_NULL })
      this.disableSaveButton = false;
      return;
    }
    if (actuator.mode === SheetMode.EditWithLock) {
      this.actuatorDataService.alterActuator(this.projectId, actuator.id, actuator.ent)
        .then(() => {
    ...
    } else if (actuator.mode === SheetMode.New) {
      this.actuatorDataService.createActuator(this.projectId, actuator.ent)
        .then(() => {
  ...
  getSelectedEntities($event: any): void {
    this.selectActuatorEntity = new SelectedEntity<Object>($event,
      this.title,
      this.projectId,
      false,
      false,
  ...
  private loadData(): void {
    ...
    this.actuatorDataService.getAllActuatorsForTable(this.projectId, {'orderBy': 'id', 'orderDirection': 'asc'})
      .then(value => {
        if (value !== null) {
  ...
  changeEntity(event: { list: ActuatorResponseDTO[], state: string }) {
    event.list.forEach((element: ActuatorResponseDTO) => {

      let incomplete: incompleteEntityDTO = { entityName: 'actuator', parentId: undefined, state: event.state };
  ...
  closeSheet(isUnlocked: boolean = false, sheetMode: SheetMode = SheetMode.Closed): void {
    if (!isUnlocked && this.sheetMode === SheetMode.Edit) {
      const unlockDTO: UnlockRequestDTO = { id: this.actuator.id, entityName: this.title.toLowerCase() };
